...
import dreamplace.ops.lp_dp.lpdp_flow.lpdp_flow as lpdp_flow
import pdb
...
class BasicPlace(nn.Module):
    ...
    def __init__(self, params, placedb):
        ...
        self.op_collections.draw_place_op = self.build_draw_placement(
            params, placedb)
        # lp_dp
        self.op_collections.lp_dp_op = self.build_lp_dp(
            params, placedb, self.data_collections, self.device)
    ...
    def build_lp_dp(self, params, placedb, data_collections, device):
        """
        @brief lp_dp 
        @param params parameters 
        @param placedb placement database 
        ...
        @param device cpu or cuda 
        """
        # for legalization and detail place
        # the number of bins control the search granularity
        lp_dp = lpdp_flow.LpdpFlow(
           num_bins_x=placedb.num_bins_x,
           num_bins_y=placedb.num_bins_y,
           flow_step=0xFF,
           save_db=1,
        ...
        def build_lp_dp_op():
            logging.info("Start legalization and detail placement")
            return lp_dp()
        return build_lp_dp_op
