...
class HPWLFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, flat_netpin, netpin_start, net_weights, net_mask, num_threads, scale):
...
class HPWLAtomicFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, pos, pin2net_map, net_weights, net_mask, scale):
        output = pos.new_empty(1)
        if pos.is_cuda:
...
class HPWL(nn.Module):
    ...
    """
    def __init__(self, flat_netpin=None, netpin_start=None, pin2net_map=None, net_weights=None, net_mask=None, algorithm='atomic', num_threads=8, scale=1.0):
        """
