...
struct CompareByNodeNTUPlaceCostBySizeCPU
{
    ...
    bool operator()(const int i, const int j) const 
    {
        T wi = 0.001*(node_x[i]+node_size_x[i]/2) + 1000*(node_size_x[i] + node_size_y[i]);
        T wj = 0.001*(node_x[j]+node_size_x[j]/2) + 1000*(node_size_x[j] + node_size_y[j]);
        return wi < wj || (wi == wj && (node_y[i] < node_y[j] || (node_y[i] == node_y[j] && i < j))); 
    }
...
template <typename T>
struct CompareByNodeNTUPlaceCostBySizeFromLeftCPU 
{
    CompareByNodeNTUPlaceCostBySizeFromLeftCPU(
            const T* x, const T* y, 
            const T* size_x, const T* size_y
            ) 
        : node_x(x)
