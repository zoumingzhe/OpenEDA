...
class PlaceObj(nn.Module):
    ...
    def __init__(self, density_weight, params, placedb, data_collections,
        ...
        if "num_bins_x" in global_place_params:
            self.num_bins_x = global_place_params["num_bins_x"]
        else: 
            self.num_bins_x = placedb.num_bins_x
        if "num_bins_y" in global_place_params:
            self.num_bins_y = global_place_params["num_bins_y"]  
        else:
        ...
        self.debug_cell_names = []
        self.debug_cell_index = []
        if 'debug_cell_names' in global_place_params:
            self.debug_cell_names = global_place_params['debug_cell_names']
            self.debug_cell_index = placedb.get_cell_index(self.debug_cell_names)
        if 'debug_cell_indexes' in global_place_params:
            self.debug_cell_index.extend(global_place_params['debug_cell_indexes'])
            self.debug_cell_names.extend(placedb.get_cell_name(global_place_params['debug_cell_indexes']))
    ...
    def bin_center_x_padded(self, bin_size_x, padding):
        ...
        """
        if padding == 0:
            return torch.tensor(self.bin_center_x,
                   dtype=self.data_collections.pos[0].dtype,
                  device=self.data_collections.pos[0].device)
        else:
            xl = placedb.xl - padding * bin_size_x
            ...
            self.bin_center_x_padded = torch.from_numpy(
                placedb.bin_centers(xl, xh,
                                    bin_size_x)).to(self.device)
         
            return torch.tensor(self.bin_center_x_padded,
                   dtype=self.data_collections.pos[0].dtype,
                   device=self.data_collections.pos[0].device)
    ...
    def bin_center_y_padded(self, bin_size_y, padding):
        ...
        """
        if padding == 0:
            return torch.tensor(self.bin_center_y,
                   dtype=self.data_collections.pos[0].dtype,
                  device=self.data_collections.pos[0].device)
        else:
            yl = placedb.yl - padding * bin_size_y
            ...
            self.bin_center_y_padded = torch.from_numpy(
                placedb.bin_centers(yl, yh,
                                    bin_size_y)).to(self.device)
            return torch.tensor(self.bin_center_y_padded,
                   dtype=self.data_collections.pos[0].dtype,
                  device=self.data_collections.pos[0].device)
