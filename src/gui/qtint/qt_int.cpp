...
void QtNotifier::createFileHandler(int fd, int mask, Tcl_FileProc* proc, ClientData clientData) {
    ...
    QtFileHandler* hdlr = new QtFileHandler(getInstance(), proc, clientData, mask);
    QSocketNotifier* ntf;
...
void QtNotifier::do_tcl_callback_(int type, int fd) {
  // find the handler
  ...
  handler_it->second->callback_file_handler(type, fd);
}


void QtNotifier::readReady(int fd) { do_tcl_callback_(TCL_READABLE,fd); }
void QtNotifier::writeReady(int fd) { do_tcl_callback_(TCL_WRITABLE,fd); }
void QtNotifier::exception(int fd) { do_tcl_callback_(TCL_EXCEPTION,fd); }
...
void QtFileHandler::callback_file_handler(int type,
					int fd      
					) {
  if (!(mask_ & type)) {
...
void QtFileHandler::bind_socket_notifier(QSocketNotifier* notifier)
{
  notifier_ = notifier;
}

void QtFileHandler::delete_socket_notifier()
{
  delete notifier_;
}
